<template>
  <div>
    <b-modal ref="add-client-modal" title="Add Client">
      <b-form>
        ...
        <b-row v-for="(t, index) in transactions" :key="index">
          ...
          <b-col md="2">
            <b-button block variant="danger"><b-icon icon="trash"></b-icon></b-button>
          </b-col>
        </b-row>
      </b-form>
...
export default {
  ...
  methods: {
    ...
    async save() {
      try {
        await clientService.create({ ...this.client, transactions: [...this.transactions] });
